...
export default function PlacementsEditForm(props: { id: string }) {
  ...
  const getCities = cities
    ? cities
    : ru.data.data.region.cities.map((city: any) => ({
        value: city.id,
        label: city.title,
      }));
  ...
  useEffect(() => {
    if (editPlacementHook.isSuccess) {
      toast.update("addPlacement", {
        render: "Успешно обновлено место размещения",
        type: "success",
        isLoading: false,
        autoClose: 3000,
      });
      ...
      queryClient.invalidateQueries({
        queryKey: ["placementById", props.id + "_ru"],
      });
  ...
  const placementTypes = types.data?.data.map((type: any) => ({
    value: type.id,
    label: type.title,
  }));

  const allRegions = useRegions();
  const regions = allRegions.data?.data.map((region: any) => ({
    value: region.id,
    label: region.title,
  }));

  const allFoodTypes = useFoodTypes();
  const foodTypes = allFoodTypes.data?.data.map((type: any) => ({
    value: type.id,
    label: type.title,
  }));

  const allServiceTypes = useServiceTypes();
  const serviceTypes = allServiceTypes.data?.data.map((type: any) => ({
    value: type.id,
    label: type.title,
  }));
